...
def chunk_pages_with_metadata(
    pages_text: List[str], chunk_size: int = 800, overlap: int = 150
) -> List[Dict[str, Any]]:
    """New: like chunk_text, but keeps track of which PDF page each
    chunk came from, e.g. [{"text": "...", "page": 3}, ...].

    This is the foundation the clickable-citation feature (review PDF,
    section "Citation Clicking") needs — without page numbers attached
    to chunks, a citation can only point at a whole document, never at
    the exact page a judge would want to jump to.

    Tradeoff, stated plainly: chunks here never span a page boundary
    (each page is chunked independently), so a fact split across two
    pages could land in two separate chunks instead of one. That's the
    right tradeoff for page-accurate citations; if you'd rather keep
    chunk_text's cross-page chunking, you'd need to track a
    character-offset -> page lookup table instead, which is more work
    for the same demo-visible benefit.

    INTEGRATION: wherever main.py currently calls
    `chunk_text(full_text)` for a newly ingested document, call this
    instead with the `pages_text` list `extract_pages()` already
    returns, and store `page` alongside each chunk's embedding/id in
    Chroma metadata. retrieval.py's `sources` list can then include the
    page number, and the Flutter side can open the PDF at that page.
    """
    chunks = []
...
# VJTI AI Hackathon 2026 — Problem Area 3: repurposed from AtlasAI's
# equipment-tag pattern to HTE document reference numbers (Circular/GR/
# Notice/Order No.). Kept as EQUIPMENT_PATTERN / extract_equipment_tags
# (same names) since retrieval.py calls this by name to tag-boost a
# query — renaming would ripple into retrieval.py for no functional
# benefit. Must stay consistent with entity_extraction.py's
# REFERENCE_PATTERN so a reference number mentioned in a query
# normalizes to the same string an uploader typed into the "Reference
# No." field at ingest time.
...
def extract_equipment_tags(full_text: str) -> List[str]:
    ...
    for match in EQUIPMENT_PATTERN.finditer(full_text):
        ...
        tag = f"{kind.upper()}-{number.upper().strip('.')}"
        tags.add(tag)
    return sorted(tags)
